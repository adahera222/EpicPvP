...
private var controller : GameCharacterController;
private var velocity = Vector3.zero;
private var targetHeight = 100000.0;

function Awake ()
{
	var characterController : CharacterController = positionTarget.collider;
	if (characterController)
	{
		centerOffset = characterController.bounds.center - positionTarget.position;
		headOffset = centerOffset;
		headOffset.y = characterController.bounds.max.y - positionTarget.position.y;
	}
	
	if (positionTarget)
	{
		controller = positionTarget.GetComponent(GameCharacterController);
	}
...
function LateUpdate () {
	...
	var targetCenter = positionTarget.position + centerOffset;
	var targetHead = positionTarget.position + headOffset;
...
function ApplySnapping (targetCenter : Vector3)
{
	...
	var targetAngle = positionTarget.eulerAngles.y;
